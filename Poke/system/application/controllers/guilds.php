...
class Guilds extends Controller {
	...
	public function view($id = null, $action = 0) {
		...
		if($action == 1) { success("Você acabou de entrar na guild."); echo "<br />";}
		$ots = POT::getInstance();
		$ots->connect(POT::DB_MYSQL, connection());
		$guild = $ots->createObject('Guild');
		try {$guild->load($id); } catch(Exception $e) {show_error('Ocorreu um problema durante o carregamento da página guilds. Cód 01200: '.$e->getMessage()); }
		if(!$guild->isLoaded()) error("Essa guild não existe.");
		$data['guild'] = $guild;
		$this->load->model("guilds_model");
	...
	public function _checkPlayer($id) {
		...
		if($this->guilds_model->checkPlayerCreatingGuild($id)) {
			...
			$this->form_validation->set_message('_checkPlayer', 'Essa guild não existe.');
			return false;
		}
	}
	...
	public function _checkGuildName($name){
		...
		if($this->guilds_model->checkGuildName($name)) {
			...
			$this->form_validation->set_message('_checkGuildName', 'Já existe uma guild com esse nome.');
			return false;
		}
	}
